...
class UserAdmin(BaseUserAdmin):
    model = User
    list_display = ("id", "phone_number", "balance", "is_active", "is_staff", "date_joined")
...
class TopUpTransactionAdmin(admin.ModelAdmin):
    list_display = ("id", "user", "amount", "cashback", "is_active", "created_at", "activated_at", "receipt_link")
    ...
    def approve_topups(self, request, queryset):
        ...
        with transaction.atomic():
            for tx in queryset.select_related("user"):
                if tx.is_active:
                    continue
                total = (Decimal(tx.amount) + Decimal(tx.cashback)).quantize(Decimal('0.01'))
                user = tx.user
                ...
                Notification.objects.create(
                    ...
                    message=f'+{tx.amount} qo\'shildi, +{tx.cashback} cashback. Balans yangilandi.'
                )
                approved += 1
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change and obj.pk and 'is_active' in form.changed_data:
            ...
            if not prev.is_active and obj.is_active:
                with transaction.atomic():
                    total = (Decimal(obj.amount) + Decimal(obj.cashback)).quantize(Decimal('0.01'))
                    user = obj.user
                    user.balance = (Decimal(user.balance) + total).quantize(Decimal('0.01'))
                    user.save(update_fields=["balance"])
                    ...
                    Notification.objects.create(
                        ...
                        type='success',
                        title='Top-up approved',
                        message=f'+{obj.amount} qo\'shildi, +{obj.cashback} cashback. Balans yangilandi.'
                    )
                return
        super().save_model(request, obj, form, change)
